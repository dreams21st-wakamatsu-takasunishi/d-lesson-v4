...
過去に追加し、まだAuth User IDがない教室児童は、児童一覧のAuth欄に `未連携` と表示されます。
`Auth作成` を押して児童番号と6けた以上のあいことばを設定すると、次をまとめて行います。

- Supabase Authの児童アカウント作成
- `lesson_user_access` への生徒権限登録
- 児童データへの `loginNumber` / `authUserId` 保存
- 作成直後のログインカード表示

あいことば自体は児童データへ保存しないため、表示されたログインカードをその場で印刷してください。
