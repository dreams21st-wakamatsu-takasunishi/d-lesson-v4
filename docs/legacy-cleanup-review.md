...
- Vite初期テンプレート由来の未使用ファイル
  - `src/counter.js`
  - `src/assets/vite.svg`
  - `src/assets/javascript.svg`
  - `src/assets/hero.png`
  - いずれも現在のDレッスン画面から参照されていなかったため削除した。

- 管理者操作ログに残っていた `legacy admin` 表記
  - 操作ログの実行者表示を Supabase Auth 前提の表記へ変更した。
  - 旧パスワード方式で管理者を識別しているように見える表記をなくした。

- 管理者運用確認画面に残っていた旧パスワード確認カード
  - 旧パスワード方式は撤去済みで、運用中に確認する項目ではなくなったため画面項目から外した。
  - 旧パスワードが戻っていないかは `npm.cmd run check:legacy-password` で検出する。

- 旧 `globalMistakes` の異常値をそのまま読む挙動
  - 旧バックアップ由来で、苦手キーの回数に指数表記の巨大数やキーではない項目が混ざる場合があった。
  - レポート、マイページ、苦手克服練習、移行スクリプトで共通の正規化処理を使い、現実的な回数だけを苦手キーとして扱うようにした。

- CSVからログインカードHTMLを作る旧フロー
  - 現在は管理者画面の `生徒管理` で対象児童を選択し、`ログインカード` から印刷できる。
  - 旧フローの管理画面CSV読込UI、`build:student-login-cards` コマンド、`scripts/build-student-login-cards.mjs` を削除した。
  - `student-login-accounts.csv` は一括移行やAuth連携SQL作成でまだ使うため残す。
...
- `VITE_ENABLE_LEGACY_SUPABASE_SYNC`
  - 公開設定の切り替え漏れを検出するため、環境変数名とチェックは残す。
  - アプリ本体では常に無効扱いにし、旧式の全件同期を再有効化できないようにした。
...
- `__GLOBAL_SETTINGS__` をユーザーデータと同じテーブルに置く設計
  ...
  - 旧 `__GLOBAL_SETTINGS__` 行は互換用として残している。lesson_settings運用が安定した後にDB上から削除するか判断する。
...
4. `lesson_settings` 運用が安定したら、DB上の旧 `__GLOBAL_SETTINGS__` 行を削除するか判断する。
