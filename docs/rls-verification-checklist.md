...
ただし、policy一覧に `Allow public read` / `Allow public read test` / `Allow public update` などが残り、`roles` が `{public}`、`qual` または `with_check` が `true` の場合は危険な旧公開ポリシーが残っている。先に `supabase/sql/drop_legacy_public_policies.sql` をSQL Editorで実行してから再確認する。
...
公開URL・RLS検証時は、旧パスワード関連の変数を設定しない。
